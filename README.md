...
**Then export what the bootstrap Jobs minted.** The Jobs mint four Secrets at the default and five with `bootstrap.iamKeys.create` on. Nothing tracks any of them, which is what keeps an uninstall or a prune from deleting them — and it is also what leaves nothing backing them up. A cluster deleted without an export loses the values for good. Run these six commands after the first install, and store the output where you store your other backups. The last of them only returns something when `iam-keys` exists on the cluster.
...
kubectl -n yadgar get secret iam-keys -o yaml
```
...
**The sixth is the one whose loss destroys data, and it is exported only if you asked for it.** `iam-keys` exists on this cluster only when `bootstrap.iamKeys.create` is true, or when you minted it yourself. Losing it does not cost a rotation: every stored name is encrypted under `encryption.key` and every username is found through `blind-index.key`, so a database that outlives the Secret holds rows nothing can read. Export it first, and store it where you store the rest.
