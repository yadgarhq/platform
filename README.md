...
**Then export what the bootstrap Jobs minted.** Nothing tracks those four Secrets, which is what keeps an uninstall or a prune from deleting them — and it is also what leaves nothing backing them up. A cluster deleted without an export loses the values for good. Run these six commands after the first install, and store the output where you store your other backups. The last of them only returns something when `bootstrap.iamKeys.create` is on.
